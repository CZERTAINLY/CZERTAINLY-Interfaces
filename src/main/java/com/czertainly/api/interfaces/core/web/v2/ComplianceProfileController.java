...
public interface ComplianceProfileController extends AuthProtectedController {
    ...
    ComplianceProfileDto updateComplianceProfile(
            ...
            @RequestBody ComplianceProfileUpdateRequestDto request
    ) throws AlreadyExistException, ConnectorException, AttributeException, NotFoundException;
    ...
    List<BulkActionMessageDto> forceDeleteComplianceProfiles(@RequestBody List<UUID> uuids) throws NotFoundException, ValidationException;
    ...
    List<ComplianceRuleListDto> getComplianceRules(
    ...
    List<ComplianceGroupListDto> getComplianceGroups(
    ...
    @ResponseStatus(HttpStatus.NO_CONTENT)
    void addRemoveComplianceRule(@Parameter(description = "Compliance Profile UUID")
                                     @PathVariable UUID uuid, @RequestBody ComplianceProfileRulesPatchRequestDto request)
    ...
    @ApiResponses(value = {@ApiResponse(responseCode = "204", description = "Group is added/removed to/from the profile"),
            @ApiResponse(responseCode = "422", description = "Unprocessable Entity", content = @Content(array = @ArraySchema(schema = @Schema(implementation = String.class)),
                    examples = {@ExampleObject(value = "[\"Error Message 1\",\"Error Message 2\"]")})),})
    @PatchMapping(path = "/{uuid}/groups", consumes = {"application/json"}, produces = {"application/json"})
    @ResponseStatus(HttpStatus.NO_CONTENT)
    void addRemoveComplianceGroup(@Parameter(description = "Compliance Profile UUID")
                  @PathVariable UUID uuid, @RequestBody ComplianceProfileGroupsPatchRequestDto request)
    ...
    List<ResourceObjectDto> getAssociations(@Parameter(description = "Compliance Profile UUID")
                                                         @PathVariable UUID uuid) throws NotFoundException;
    ...
    @ResponseStatus(HttpStatus.NO_CONTENT)
    void checkResourceObjectCompliance(
            @Parameter(description = "Resource", required = true, example = Resource.Codes.RA_PROFILE) @PathVariable Resource resource,
