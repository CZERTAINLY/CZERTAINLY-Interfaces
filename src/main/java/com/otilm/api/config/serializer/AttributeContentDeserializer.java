...
public class AttributeContentDeserializer extends JsonDeserializer<AttributeContent> {
    ...
    @Override
    public AttributeContent deserialize(JsonParser jsonParser, DeserializationContext deserializationContext)
            throws IOException {
        ...
        if (hasContentType && !node.get(CONTENT_TYPE).isNull()) {
            ...
            return jsonParser.getCodec().treeToValue(node, BaseAttributeContentV2.class);
        }
    }
}
