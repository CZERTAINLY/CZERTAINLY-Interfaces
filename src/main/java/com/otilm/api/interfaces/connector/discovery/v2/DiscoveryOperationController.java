...
@Tag(name = "Discovery Operations v2", description = "Stateless discovery v2 run lifecycle: initiate a run, poll or stream its results, "
...
public interface DiscoveryOperationController extends AuthProtectedConnectorController {
    ...
    @Operation(summary = "Initiate a discovery run", description = "Starts a new discovery run for the given resource types and attributes. runId is "
    ...
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "422", description = "One or more requested resources or attributes are unsupported or invalid (errorCode VALIDATION_FAILED)", content = @Content(mediaType = MediaType.APPLICATION_PROBLEM_JSON_VALUE, schema = @Schema(implementation = ProblemDetailExtended.class)))})
    ...
    @Operation(summary = "Get discovery run status", description = "Returns the run's current state and progress.")
    @ApiResponses({@ApiResponse(responseCode = "200", description = "Run status retrieved"),
            @ApiResponse(responseCode = "404", description = "Run not tracked by the connector (errorCode OPERATION_NOT_TRACKED)", content = @Content(mediaType = MediaType.APPLICATION_PROBLEM_JSON_VALUE, schema = @Schema(implementation = ProblemDetailExtended.class)))})
    ...
    @Operation(summary = "Drain discovered items", description = "Returns one page of items discovered after afterSequence, bounded by maxItems "
            ...
            + "state. Once the full ack arrives, the connector MAY discard all state it holds for the " + "run.")
    @ApiResponses({
    ...
    @Operation(summary = "Stream discovery events", description = "Opens a chunked application/x-ndjson response, one JSON event object per line, "
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "NDJSON event stream opened; the body is one flat JSON "
                    + "DiscoveryEvent object per line, never a JSON array of events", content = @Content(mediaType = MediaType.APPLICATION_NDJSON_VALUE, schema = @Schema(implementation = DiscoveryEvent.class))),
            @ApiResponse(responseCode = "404", description = "Run not tracked by the connector (errorCode OPERATION_NOT_TRACKED)", content = @Content(mediaType = MediaType.APPLICATION_PROBLEM_JSON_VALUE, schema = @Schema(implementation = ProblemDetailExtended.class)))})
    ...
    @Operation(summary = "Stop a discovery run", description = "Halts scanning for an in-progress run. The connector persists a best-effort "
    ...
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "404", description = "Run not tracked by the connector (errorCode OPERATION_NOT_TRACKED)", content = @Content(mediaType = MediaType.APPLICATION_PROBLEM_JSON_VALUE, schema = @Schema(implementation = ProblemDetailExtended.class))),
            @ApiResponse(responseCode = "422", description = "Run cannot be stopped: unsupported capability for its resource types, or not in a stoppable state (errorCode OPERATION_PAST_POINT_OF_NO_RETURN)", content = @Content(mediaType = MediaType.APPLICATION_PROBLEM_JSON_VALUE, schema = @Schema(implementation = ProblemDetailExtended.class)))})
    ...
    @Operation(summary = "Resume a stopped discovery run", description = "Continues a stopped run from the checkpoint carried in the replayed meta. Item "
    ...
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "404", description = "Run not tracked by the connector (errorCode OPERATION_NOT_TRACKED)", content = @Content(mediaType = MediaType.APPLICATION_PROBLEM_JSON_VALUE, schema = @Schema(implementation = ProblemDetailExtended.class))),
            @ApiResponse(responseCode = "410", description = "The checkpoint needed to resume this run is no longer available (errorCode CHECKPOINT_LOST); distinct from 404, which means the run was never known", content = @Content(mediaType = MediaType.APPLICATION_PROBLEM_JSON_VALUE, schema = @Schema(implementation = ProblemDetailExtended.class)))})
    ...
    @Operation(summary = "Cancel a discovery run", description = "Terminally aborts the run; the connector discards its job state for this runId "
            + "once it has answered.")
    @ApiResponses({@ApiResponse(responseCode = "204", description = "Run cancelled"),
            @ApiResponse(responseCode = "404", description = "Run not tracked by the connector (errorCode OPERATION_NOT_TRACKED); treat as an already-terminal cancellation rather than an error", content = @Content(mediaType = MediaType.APPLICATION_PROBLEM_JSON_VALUE, schema = @Schema(implementation = ProblemDetailExtended.class))),
            @ApiResponse(responseCode = "422", description = "Cancel refused: the run is past the point of no return (errorCode OPERATION_PAST_POINT_OF_NO_RETURN)", content = @Content(mediaType = MediaType.APPLICATION_PROBLEM_JSON_VALUE, schema = @Schema(implementation = ProblemDetailExtended.class)))})
