...
public interface DiscoveryController extends AuthProtectedConnectorController {
    ...
    @PostMapping(path = "/discover/{uuid}", consumes = {"application/json"}, produces = {"application/json"})
    ...
    DiscoveryProviderDto getDiscovery(@Parameter(description = "Discovery UUID") @PathVariable String uuid,
            @RequestBody DiscoveryDataRequestDto request) throws IOException, NotFoundException;

    @DeleteMapping(path = "/discover/{uuid}", produces = {"application/json"})
    @Operation(summary = "Delete Discovery")
    @ApiResponses(value = {@ApiResponse(responseCode = "204", description = "Discovery information deleted")})
    @ResponseStatus(HttpStatus.NO_CONTENT)
    void deleteDiscovery(@Parameter(description = "Discovery UUID") @PathVariable String uuid)
            throws IOException, NotFoundException;
