 ...
 *
 * <p>
 * Token requests are scoped by token and token-profile attributes supplied in the request body.
 * </p>
 */
@RequestMapping("/v2/cryptographyProvider/tokens")
@Tag(name = "Token Management v2", description = "Token configuration, status and token-profile schema operations for cryptography providers")
public interface TokenController extends AuthProtectedConnectorController {
    ...
    @Operation(summary = "Check token status", description = "Evaluates the supplied token context and returns token reachability, readiness, "
            + "and lockout information when available")
    @ApiResponses({@ApiResponse(responseCode = "200", description = "Token status retrieved"),
            @ApiResponse(responseCode = "422", description = "Request validation failed (errorCode VALIDATION_FAILED)", content = @Content(mediaType = MediaType.APPLICATION_PROBLEM_JSON_VALUE, schema = @Schema(implementation = ProblemDetailExtended.class)))})
    ...
    @Operation(summary = "List token profile attributes", description = "Returns the token profile attribute schema for the supplied token context")
    ...
    @Operation(summary = "List supported token profile key usages", description = "Returns the key usages supported for the supplied token context")
    ...
    @Operation(summary = "List key request types supported by the Token Profile", description = "List key types supported for the supplied token and token-profile context")
