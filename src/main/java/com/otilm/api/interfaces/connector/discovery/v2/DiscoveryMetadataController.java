...
@Tag(name = "Discovery Metadata v2", description = "Stateless discovery v2 metadata surface: which resource types and capabilities "
...
public interface DiscoveryMetadataController extends AuthProtectedConnectorController {
    ...
    @Operation(summary = "List supported resources", description = "Returns the resource types this connector can discover, and which capabilities "
    ...
    @Operation(summary = "List run-level attributes", description = "Returns the attribute schema that configures a discovery run as a whole and "
    ...
    /**
     * The {@code resource} path segment binds by the {@link Resource} wire code (e.g. {@code "certificates"},
     * {@code "keys"}), not the Java enum member name — the same converter requirement carried by the platform's other
     * kind-in-path controllers (see the secrets v1 precedent, {@code SecretController#getSecretAttributes}). Register
     * {@code com.otilm.api.config.converter.IPlatformEnumConverterFactory} (for example via
     * {@code WebMvcConfigurer#addFormatters}) to bind this and every other {@code IPlatformEnum}-typed path variable by
     * code without writing a converter by hand.
     */
    @Operation(summary = "List per-resource attributes", description = "Returns the attribute schema that refines discovery of a single resource type. "
