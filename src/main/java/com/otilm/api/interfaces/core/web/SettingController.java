...
public interface SettingController extends AuthProtectedController {
     ...
     */
    @Operation(summary = "Update platform branding", description = "The only way to write branding.\n\n"
            + "**Authorization:** gated by `UPDATE_BRANDING`, a narrower action than the `UPDATE` that gates "
            + "the rest of settings. Authorization is applied per endpoint. Branding is therefore "
            + "deliberately absent from the `PUT /v1/settings/platform` body. Carried there, it would be "
            + "writable by anyone holding plain `UPDATE`, leaving `UPDATE_BRANDING` nothing extra to " + "grant.\n\n"
