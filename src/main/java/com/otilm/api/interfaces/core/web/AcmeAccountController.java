...
public interface AcmeAccountController extends AuthProtectedController {
    ...
    @GetMapping(path = "/acmeProfiles/{acmeProfileUuid}/acmeAccounts/{acmeAccountUuid}", produces = {
            "application/json"})
    AcmeAccountResponseDto getAcmeAccount(
            @Parameter(description = "ACME Profile UUID") @PathVariable String acmeProfileUuid,
            @Parameter(description = "ACME Account UUID") @PathVariable String acmeAccountUuid)
    ...
    @PatchMapping(path = "/acmeProfiles/{acmeProfileUuid}/acmeAccounts/{acmeAccountUuid}/enable", produces = {
            "application/json"})
    @ResponseStatus(HttpStatus.NO_CONTENT)
    void enableAcmeAccount(@Parameter(description = "ACME Profile UUID") @PathVariable String acmeProfileUuid,
            @Parameter(description = "ACME Account UUID") @PathVariable String acmeAccountUuid)
            throws NotFoundException;

    @Operation(summary = "Disable ACME Account")
    @ApiResponses(value = {@ApiResponse(responseCode = "204", description = "ACME Account disabled")})
    @PatchMapping(path = "/acmeProfiles/{acmeProfileUuid}/acmeAccounts/{acmeAccountUuid}/disable", produces = {
            "application/json"})
    @ResponseStatus(HttpStatus.NO_CONTENT)
    void disableAcmeAccount(@Parameter(description = "ACME Profile UUID") @PathVariable String acmeProfileUuid,
            @Parameter(description = "ACME Account UUID") @PathVariable String acmeAccountUuid)
            throws NotFoundException;

    @Operation(summary = "Revoke ACME Account")
    @ApiResponses(value = {@ApiResponse(responseCode = "204", description = "ACME Account revoked")})
    @PostMapping(path = "/acmeProfiles/{acmeProfileUuid}/acmeAccounts/{acmeAccountUuid}", produces = {
            "application/json"})
    @ResponseStatus(HttpStatus.NO_CONTENT)
    void revokeAcmeAccount(@Parameter(description = "ACME Profile UUID") @PathVariable String acmeProfileUuid,
            @Parameter(description = "ACME Account UUID") @PathVariable String acmeAccountUuid)
            throws NotFoundException;
    ...
    @ResponseStatus(HttpStatus.NO_CONTENT)
    void bulkEnableAcmeAccount(
            @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "ACME Account UUIDs", content = @Content(array = @ArraySchema(schema = @Schema(implementation = String.class)), examples = {
