...
/**
 * This exception is created if an enrollment (scope: cmp request, types: ir, cr, kur) is failed or in corrupted state.
 */
public class CmpCrmfValidationException extends CmpProcessingException {
     ...
     */
    public CmpCrmfValidationException(ASN1OctetString tid, int typeOfRequestEnrollment, int failureInfo,
            String errorDetails) {
    ...
    public PKIBody toPKIBody() {
        return PkiMessageError.generateCrmfErrorBody(this.typeOfRequestEnrollment + 1, failureInfo, errorDetails);
    }
}
