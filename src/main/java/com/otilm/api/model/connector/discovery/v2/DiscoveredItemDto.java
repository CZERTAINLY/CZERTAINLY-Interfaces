...
/**
 * A single item produced by a discovery run: one certificate or one cryptographic key found
 * by the connector.
 *
 * <p>{@code resource} is not a field of this container: it is {@code payload}'s own discriminator
 * (see {@link DiscoveredItemPayloadDto}). {@link #getResource()} is a derived, read-only,
 * {@code @JsonIgnore}d convenience that unwraps it.
 */
@Getter
@Setter
@ToString
...
public class DiscoveredItemDto {
    ...
    /**
     * Derived from {@code payload}, null-safe, and excluded from the wire (see class javadoc).
     * Not a field: there is nothing to set independently of {@code payload}, and a setter would
