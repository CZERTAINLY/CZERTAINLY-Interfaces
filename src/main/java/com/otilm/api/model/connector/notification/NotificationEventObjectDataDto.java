...
public class NotificationEventObjectDataDto {
    ...
    @Schema(description = "The object this data describes: the event object, or for approval events the approval's target object",
            requiredMode = Schema.RequiredMode.REQUIRED)
    private NotificationAssociationDto subject;

    @Schema(description = "The subject's custom attributes keyed by attribute name",
            requiredMode = Schema.RequiredMode.NOT_REQUIRED)
    private Map<String, NotificationAttributeDto> customAttributes;

    @Schema(description = "The subject's connector-sourced metadata, grouped by connector and source object type",
            requiredMode = Schema.RequiredMode.NOT_REQUIRED)
    private List<NotificationMetadataGroupDto> metadata;

    @Schema(description = "The subject's associated objects: owner, groups, RA profile",
            requiredMode = Schema.RequiredMode.NOT_REQUIRED)
    private List<NotificationAssociationDto> associations;

    @Schema(description = "The subject's content representation when its resource type provides one",
            requiredMode = Schema.RequiredMode.NOT_REQUIRED)
