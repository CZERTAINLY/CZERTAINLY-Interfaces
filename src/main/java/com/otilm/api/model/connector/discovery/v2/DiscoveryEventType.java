...
public enum DiscoveryEventType implements IPlatformEnum {
    ...
    /**
     * Compile-time constants for {@code @JsonSubTypes.Type(name = ...)} on {@link DiscoveryEvent},
     * mirroring {@code Resource.Codes} and {@code AttributeVersion.Codes} — an annotation attribute
     * must be a constant expression, so the enum's own {@link #code} field cannot be referenced
     * there directly.
     */
