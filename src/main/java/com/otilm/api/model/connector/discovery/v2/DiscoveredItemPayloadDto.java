...
@JsonSubTypes({@JsonSubTypes.Type(value = DiscoveredCertificateDto.class, name = Resource.Codes.CERTIFICATE),
        @JsonSubTypes.Type(value = DiscoveredKeyDto.class, name = Resource.Codes.CRYPTOGRAPHIC_KEY)})
@Schema(name = "DiscoveredItemPayload", description = "Resource-specific payload of a discovered item. The required resource property is "
        + "the discriminator: it carries the resource's wire code and selects which concrete "
        + "payload shape this object is. Each concrete shape fixes it to its own value.", type = "object", discriminatorProperty = "resource", discriminatorMapping = {
                @DiscriminatorMapping(value = Resource.Codes.CERTIFICATE, schema = DiscoveredCertificateDto.class),
                @DiscriminatorMapping(value = Resource.Codes.CRYPTOGRAPHIC_KEY, schema = DiscoveredKeyDto.class)}, oneOf = {
                        DiscoveredCertificateDto.class, DiscoveredKeyDto.class})
