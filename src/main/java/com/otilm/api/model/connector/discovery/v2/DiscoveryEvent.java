...
/**
 * One event on a held-open NDJSON discovery stream — one flat JSON object per line — also used as
 * the payload of the AMQP {@code discovery.event} message.
 *
 * <p>{@code type} is this interface's own discriminator, declared on each subtype rather than on a
 * wrapper. Do not add a {@code {type, payload}} wrapper around it — that would only duplicate the
 * discriminator and let the two copies disagree.
 */
...
@Schema(
        name = "DiscoveryEvent",
        description = "One flat discovery stream/AMQP event; type selects the concrete shape.",
        type = "object",
