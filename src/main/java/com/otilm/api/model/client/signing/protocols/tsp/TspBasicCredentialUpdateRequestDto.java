...
public class TspBasicCredentialUpdateRequestDto {
    ...
    @NotBlank
    @Schema(description = "Basic username; unique within the TSP profile. Changing it requires a new password.", requiredMode = Schema.RequiredMode.REQUIRED, example = "svc-account")
    private String username;

    @ToString.Exclude
    @JsonProperty(access = JsonProperty.Access.WRITE_ONLY)
    @Schema(description = "Basic password (write-only). Blank keeps the existing secret and a value rotates it; "
            + "a value is also required when the username changes.", requiredMode = Schema.RequiredMode.NOT_REQUIRED, accessMode = Schema.AccessMode.WRITE_ONLY)
