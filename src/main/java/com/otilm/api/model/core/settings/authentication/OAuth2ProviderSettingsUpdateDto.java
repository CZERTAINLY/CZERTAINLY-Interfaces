...
public class OAuth2ProviderSettingsUpdateDto implements Serializable {
    ...
    @Schema(description = "The client ID used to identify the client application during the authorization process.",
            requiredMode = Schema.RequiredMode.NOT_REQUIRED)
    ...
    @URL
    @Schema(description = "The URL where the authorization server redirects the user for login and authorization.",
            requiredMode = Schema.RequiredMode.NOT_REQUIRED)
    private String authorizationUrl;

    @URL
    @Schema(description = "The URl used to exchange the authorization code or credentials for an access token.",
            requiredMode = Schema.RequiredMode.NOT_REQUIRED)
    private String tokenUrl;

    @URL
    @Schema(description = "The URL where the JSON Web Key Set (JWKS) containing the public keys used to verify JWT tokens can be retrieved.",
            requiredMode = Schema.RequiredMode.NOT_REQUIRED)
    private String jwkSetUrl;

    @Schema(description = "Base64 encoded JWK Set, provided in case JWK Set URL is not available",
            requiredMode = Schema.RequiredMode.NOT_REQUIRED)
    ...
    @URL
    @Schema(description = "URL that user will be redirected after logout from application.",
            requiredMode = Schema.RequiredMode.NOT_REQUIRED)
    ...
    @Schema(description = "Duration, in seconds, after which an inactive user's session will be terminated. Default value is 900 seconds.",
            defaultValue = "900", requiredMode = Schema.RequiredMode.NOT_REQUIRED)
    private int sessionMaxInactiveInterval = 15 * 60;

    @Schema(description = "The claim name used to extract the username from the token.",
            requiredMode = Schema.RequiredMode.NOT_REQUIRED)
