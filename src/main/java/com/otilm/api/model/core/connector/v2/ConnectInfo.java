...
@JsonSubTypes({@JsonSubTypes.Type(value = ConnectInfoV1.class, name = ConnectorVersion.Codes.V1),
...
public abstract class ConnectInfo {
    ...
    @Schema(description = "Version of the Connector.", requiredMode = Schema.RequiredMode.REQUIRED, example = ConnectorVersion.Codes.V2)
    private ConnectorVersion version;

    @Schema(description = "Connector UUID. Present when connector with that version is already added.", requiredMode = Schema.RequiredMode.NOT_REQUIRED)
    private UUID connectorUuid;

    @Schema(description = "Error message in case of connection failure.", requiredMode = Schema.RequiredMode.NOT_REQUIRED)
