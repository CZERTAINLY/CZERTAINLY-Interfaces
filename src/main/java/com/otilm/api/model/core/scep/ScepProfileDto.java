...
public class ScepProfileDto extends NameAndUuidDto {
    ...
    @Schema(description = "SCEP URL", examples = {
            "https://some-server.com/api/v1/protocols/scep/profile/pkiclient.exe"})
    ...
    @Schema(description = "Whether a challenge password is currently set for the SCEP Profile. The password value "
            + "itself is write-only and never returned; this flag lets clients reflect the current state.", requiredMode = Schema.RequiredMode.REQUIRED)
